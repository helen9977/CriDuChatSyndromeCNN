...
def read_to_dictionary(read_package, scan_r_pos,height):
	...
	for base_and_read in read_package:
		...
		#	print (read.is_proper_pair)
			...
			if is_empty(read_list):
				read_list[row_ptr].append( (read_pos1,read_pos2,base,quality,is_clipped) )
			else:
...
def draw_pgn(dic,width,height,scan_l_pos,scan_r_pos,img_name):
		...
		for read_tuple in dic[key]:
			...
			for i in range(len(base)):
				if col >=0  and col < width :
					row=key
				#	print ( get_RGB(base[index_in_read],quality[index_in_read],is_clipped))		
					red,green,blue=get_RGB(base[index_in_read],quality[index_in_read],is_clipped)
...
def get_RGB(base,quality,is_clipped):
...
'''
		...
		blue=0
		if quality>=1:
			red=red+255-10*quality
			blue=blue+255-10*quality
...
def main():
	...
	vcf_path='/mnt/hde/gao/helen/vcf0.txt'
	bam_path='/mnt/xiaohuanghuang/3GS_data/NA12878/ont/na12878_ont.chr5.bam'
	...
	#	chrom=str(tmp[0])
		...
		img_tmp='/mnt/hde/gao/helen/picture/LR11/'
		img_name=img_tmp+tmp[1]+'_LR_'+label+'.png'
		...
		if not c==0 :
			draw_pgn(dic,width,height,scan_l_pos,scan_r_pos,img_name)
